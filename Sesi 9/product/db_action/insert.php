<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors)) {
            ...
            if (!move_uploaded_file($image_tmp, $upload_dir . basename($image['name']))) {
                $errors['image'] = 'Gagal mengunggah gambar.';
            } else {
                $image_file = $upload_dir . basename($image['name']);
                $image_name = convert_to_avif($image_file);
                @unlink($image_file);

                if ($image_name === false) {
                    $errors['image'] = 'Gagal mengonversi gambar ke format AVIF.';
                } else {
